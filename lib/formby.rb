...
module Formby
  ...
  class Input < Base
    needs :id=>nil,:value=>nil,:style=>nil,:readonly=>false,:class=>nil,:attributes=>nil
    def classes
      if @class.respond_to?(:first) then
        @class.join(" ")
      else
        @class
      end
    ...
    def standard_attributes
      default={  :name=>self.fullname, :id=>(@id and @id.to_s),:value=>@value, :type=>self.type, :style=>@style, :readonly=>@readonly, :class=>self.classes }
      if @attributes.present? then
